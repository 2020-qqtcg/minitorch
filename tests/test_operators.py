...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    # Due to precision issues, add an equality sign.
    assert sigmoid(a + 1e-6) - sigmoid(a) >= 0.0
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if (nums[0] < nums[1]) and (nums[1] < nums[2]):
        assert (lt(nums[0], nums[1]) and lt(nums[1], nums[2])) == lt(nums[0], nums[2])
